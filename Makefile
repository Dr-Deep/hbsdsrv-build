GOCMD=go
BINARY_NAME=hbsdsrv-build
SERVICE_PATH=/usr/local/etc/rc.d/hbsdsrv-builder
BUILD_DIR=bin
SRC=.
...
install: build
	...
	@ln -vs $(BUILD_DIR)/$(BINARY_NAME) /usr/local/bin/$(BINARY_NAME)

service: install
	@echo "Installing new Service File"
	@cp -vf ./hbsdsrv_builder.service $(SERVICE_PATH)
